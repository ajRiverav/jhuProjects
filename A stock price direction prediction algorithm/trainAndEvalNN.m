...
for i=1:length(trainingAlgorithms)
    ...
    for numNeuronsHidLayer= [10]
        ...
        %Now evaluate performance with test set
        predLabels = nn(scaledExemplars(:, testSetIdx));
               
        %Write results to a file
        [FAR,TPR,T,AUC] = perfcurve(targetLabels(testSetIdx),predLabels,1);
        
        %Write results to a file
        % The data here allows us to construct a ROC curve
        % which is uncommon for NNs. There is a paper on how this is done.
